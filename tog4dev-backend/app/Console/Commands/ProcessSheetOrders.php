...
class ProcessSheetOrders extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:process-sheet-orders';
     ...
     * @var string
     */
    protected $description = 'fetch orders from sheet to create user, payments';

    /**
     * Execute the console command.
     */
    public function handle(){
        // Fetch orders where related ExcelSheet has status = 2
        $orders = ExcelOrders::whereHas('excelSheet', function ($query) {
            $query->where('status', 2);  // ExcelSheet status is 2
        })   
        ...
        if ($orders->isEmpty()) {
            $this->info('No orders found with related ExcelSheet status = 2.');
            return;
        }
    ...
    public function createOrder($order, $user){
        ...
        if(count($cart) > 0){
            ...
            $this->info("order created successfully: " . $order["order_id"]);            
            return true;
        } else {
