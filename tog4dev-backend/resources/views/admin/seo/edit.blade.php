...
<div class="row">
    <div class="col-12">
        <div class="widget-rounded-circle card-box d-flex justify-content-between">
            <form class='w-100' action="{{ route('seo.update', $seo->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" id="meta_title" name="meta_title" placeholder='{{ __('app.title') }} (AR)' value="{{ $seo->meta_title }}" class="form-control">
                            </div>

                            <!-- Title (EN) -->
                            <div class="form-group col-md-6">
                                <label for="meta_title_en">{{ __('app.title') }} (EN) (50–60 characters)</label>
                                <input type="text" id="meta_title_en" name="meta_title_en" placeholder='{{ __('app.title') }} (EN)' value="{{ $seo->meta_title_en }}" class="form-control">
                            </div>

                            <!-- Description (AR) -->
                            <div class="form-group col-md-6">
                                <label for="meta_description">{{ __('app.description') }} (AR) (150–160 characters)</label>
                                <textarea id="meta_description" name="meta_description" rows="4" placeholder='{{ __('app.description') }} (AR)' class="form-control">{{ $seo->meta_description }}</textarea>
                            </div>

                            <!-- Description (EN) -->
                            <div class="form-group col-md-6">
                                <label for="meta_description_en">{{ __('app.description') }} (EN) (150–160 characters)</label>
                                <textarea id="meta_description_en" name="meta_description_en" rows="4" placeholder='{{ __('app.description') }} (EN)' class="form-control">{{ $seo->meta_description_en }}</textarea>
                            </div>

                            <!-- Keywords (AR) -->
                            <div class="form-group col-md-6">
                                <label for="meta_keywords">{{ __('app.keywords') }} (AR) (5–10 Words ≤ 255 characters)</label>
                                <textarea id="meta_keywords" name="meta_keywords" rows="4" placeholder='{{ __('app.keywords') }} (AR)' class="form-control">{{ $seo->meta_keywords }}</textarea>
                            </div>

                            <!-- Keywords (EN) -->
                            <div class="form-group col-md-6">
                                <label for="meta_keywords_en">{{ __('app.keywords') }} (EN) (5–10 Words ≤ 255 characters)</label>
                                <textarea id="meta_keywords_en" name="meta_keywords_en" rows="4" placeholder='{{ __('app.keywords') }} (EN)' class="form-control">{{ $seo->meta_keywords_en }}</textarea>
                            </div>
