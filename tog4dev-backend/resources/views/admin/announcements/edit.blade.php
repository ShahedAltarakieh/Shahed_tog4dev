                ...
                <div class="card-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select id="source_type" name="source_type" class="form-control">
                            <option value="manual" {{ old('source_type', $announcement->source_type) == 'manual' ? 'selected' : '' }}>{{ __('app.manual_announcement') }}</option>
                            <option value="news" {{ old('source_type', $announcement->source_type) == 'news' ? 'selected' : '' }}>{{ __('app.linked_news') }}</option>
                        </select>
                    </div>
                            ...
                            <select id="news_id" name="news_id" class="form-control">
                                ...
                                @foreach($newsItems as $news)
                                    <option value="{{ $news->id }}"
                                        ...
                                        {{ old('news_id', $announcement->news_id) == $news->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" id="ann-title" name="title" class="form-control" value="{{ old('title', $announcement->title) }}">
                    </div>

                    <div class="form-group mb-3">
                        <label class="form-label" style="font-weight:600;font-size:13px;">{{ __('app.text') }} <span class="text-danger">*</span></label>
                        <textarea id="ann-text" name="text" class="form-control" rows="3" required>{{ old('text', $announcement->text) }}</textarea>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <input type="text" id="ann-link" name="link" class="form-control" value="{{ old('link', $announcement->link) }}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group mb-3">
            ...
            <div class="card" style="border-left:3px solid var(--admin-primary, #13585D);">
                <div class="card-body" style="padding:16px 20px;">
                    ...
                    <div id="live-preview" style="background:#13585D;color:#fff;padding:10px 24px;border-radius:8px;display:flex;align-items:center;justify-content:center;gap:12px;font-size:14px;min-height:44px;flex-wrap:wrap;">
                        <span id="preview-badge" class="badge" style="font-size:11px;padding:3px 8px;background:#ef4444;">{{ $announcement->badge_type }}</span>
                        <span id="preview-text" style="flex:1;text-align:center;">{{ Str::limit($announcement->text, 100) }}</span>
                        <a href="#" id="preview-cta" style="color:#FECD0F;font-weight:600;font-size:13px;text-decoration:none;{{ $announcement->cta_text ? '' : 'display:none;' }}">{{ $announcement->cta_text ?? __('app.explore_now') }} →</a>
                    </div>
