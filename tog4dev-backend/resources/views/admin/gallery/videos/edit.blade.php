                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <select name="display_target" id="display_target" class="form-control" required>
                                    <option value="both" {{ old('display_target', $data->display_target) === 'both' ? 'selected' : '' }}>
                                        {{ __('app.both') ?? 'Both (Mobile & Desktop)' }}
                                    </option>
                                    <option value="mobile" {{ old('display_target', $data->display_target) === 'mobile' ? 'selected' : '' }}>
                                        {{ __('app.mobile only') ?? 'Mobile Only' }}
                                    </option>
                                    <option value="desktop" {{ old('display_target', $data->display_target) === 'desktop' ? 'selected' : '' }}>
                            ...
                            <div class="form-group col-md-6 d-flex align-items-center">
                                @if($data->thumbnail)
                                <div id="thumbnail-preview-wrapper" style="width: 100%; text-align: center;">
                                    <p class="text-muted mb-2" style="font-size: 13px;"><i class="fas fa-image mr-1"></i>{{ __('app.current cover') ?? 'Current Cover' }}</p>
                                    <img id="thumbnail-preview" src="{{ $data->thumbnail }}" alt="Cover preview" style="max-width: 100%; max-height: 200px; border-radius: 8px; border: 2px solid #e2e8f0; object-fit: cover;" />
                                </div>
                                @else
...
document.getElementById('thumbnail').addEventListener('change', function(e) {
    ...
    if (file && file.type.startsWith('image/')) {
        ...
        reader.onload = function(ev) {
            ...
            if (label) label.textContent = '{{ __("app.new cover preview") ?? "New Cover Preview" }}';
        };
